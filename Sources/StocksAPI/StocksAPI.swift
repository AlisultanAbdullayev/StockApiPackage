...
public struct StocksAPI {
    ...
    public func fetchChartData(symbol: String, range: ChartRange) async throws -> ChartData? {
        guard var urlComponents = URLComponents(string: "\(baseURL)/v8/finance/chart/\(symbol)")
        else {
        ...
        if let error = response.error {
            throw APIError.httpStatusCodeFailed(statutCode: statusCode, error: error)
        }
    ...
    public func searchTickers(query: String, isEquityTypeOnly: Bool = true) async throws -> [Ticker] {
        guard var urlComponents = URLComponents(string: "\(baseURL)/v1/finance/search")
        else {
            throw APIError.invalidURL
        }
    ...
    public func fetchQuotes(symbols: String) async throws -> [Quote] {
        guard var urlComponents = URLComponents(string: "\(baseURL)/v7/finance/quote")
        else {
            throw APIError.invalidURL
        }
    ...
    private func validateHTTPResponse(_ response: URLResponse) throws -> Int {
        ...
        guard 200...299 ~= httpResponse.statusCode || 400...499 ~= httpResponse.statusCode
            ...
            throw APIError.httpStatusCodeFailed(statutCode: httpResponse.statusCode, error: nil)
        }
