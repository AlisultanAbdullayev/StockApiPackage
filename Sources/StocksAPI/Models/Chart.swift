...
public struct ChartData: Decodable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        if
            let indicatorsContainer = try? container.nestedContainer(keyedBy: IndicatorsKeys.self, forKey: .indicators),
            ...
            indicators = timestamps
                .enumerated()
                .compactMap { (offset, timestamp) in
...
public struct ChartMeta: Decodable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        previousClose = try container.decodeIfPresent(Double.self, forKey: .previousClose) ?? 0.0
        ...
        let regularTradingPeriodContriner = try? currentTradingPeriodContainer?.nestedContainer(keyedBy: TradingPeriodKeys.self, forKey: .regular)
        regularTradingPeriodStartDate = try regularTradingPeriodContriner?.decodeIfPresent(Date.self, forKey: .start) ?? Date()
        regularTradingPeriodEndDate = try regularTradingPeriodContriner?.decodeIfPresent(Date.self, forKey: .end) ?? Date()
